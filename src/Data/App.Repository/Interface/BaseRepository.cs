...
namespace App.Repository.Interface
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class, IEntity, new()
    {
        ...
        {
            _logger = logger.CreateLogger<BaseRepository<TEntity>>();
            _context = context;
        }
        ...
        /// <param name="strOrderByFileds">排序字段，如name asc,age desc</param>
        /// <returns></returns>
        public async Task<QuaryPageModel<TEntity>> QueryPage(Expression<Func<TEntity, bool>> whereExpression, int intPageIndex = 1, int intPageSize = 20, string strOrderByFileds = null)
        {
            ...
            return new QuaryPageModel<TEntity>() { DataCount = totalCount, PageCount = pageCount, Page = intPageIndex, PageSize = intPageSize, Data = list };
        }
        ...
        {
            return (await db.Queryable<TEntity>().In(id).CountAsync()) > 0;
        }

        public async Task<bool> Contains(Expression<Func<TEntity, bool>> whereExpression)
        {
            if (whereExpression == null)
            {
                return false;
            }
            return (await db.Queryable<TEntity>().Where(whereExpression).CountAsync()) > 0;
        }
