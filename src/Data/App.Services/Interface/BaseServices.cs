...
{
    public class BaseServices<TEntity> : IBaseServices<TEntity> where TEntity : class, IEntity, new()
    {
        public IBaseRepository<TEntity> BaseDal { get; private set; }

        public BaseServices(IBaseRepository<TEntity> _baseDal)
        {
            this.BaseDal = _baseDal;
        }
        ...
        public async Task<QuaryPageModel<TEntity>> QueryPage(Expression<Func<TEntity, bool>> whereExpression,
        ...
        {
            return await BaseDal.Contains(id);
        }

        public async Task<bool> Contains(Expression<Func<TEntity, bool>> whereExpression)
        {
            return await BaseDal.Contains(whereExpression);
        }
    }
}
